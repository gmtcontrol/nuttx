/****************************************************************************
 * boards/risc-v/sty32c2/ulx3s/src/ulx3s_automount.c
 *
...
struct ulx3s_automount_state_s
{
...
struct ulx3s_automount_config_s
{
  /* This must be first thing in structure so that we can simply cast from
   * struct automount_lower_s to struct ulx3s_automount_config_s
   */

  struct automount_lower_s lower;        /* Publicly visible part */
  uint8_t mmcsd;                         /* MB1_MMCSD_SLOTNO or MB2_MMCSD_SLOTNO */
  struct ulx3s_automount_state_s *state; /* Changeable state */
};
...
static int  ulx3s_attach(const struct automount_lower_s *lower,
                         automount_handler_t isr, void *arg);
static void ulx3s_enable(const struct automount_lower_s *lower,
                         bool enable);
static bool ulx3s_inserted(const struct automount_lower_s *lower);
...
static struct ulx3s_automount_state_s g_mb1_mmcsdstate;
static const struct ulx3s_automount_config_s g_mb1_mmcsdconfig =
{
  .lower        =
  {
...
static int ulx3s_attach(const struct automount_lower_s *lower,
                      automount_handler_t isr, void *arg)
{
  const struct ulx3s_automount_config_s *config;
  struct ulx3s_automount_state_s *state;

  /* Recover references to our structure */

  config = (struct ulx3s_automount_config_s *)lower;
  ...
  if (enable && state->pending)
    {
      ...
      if (state->handler)
        {
          bool inserted = ulx3s_cardinserted(config->mmcsd);
...
static bool ulx3s_inserted(const struct automount_lower_s *lower)
{
  ...
  return ulx3s_cardinserted(config->mmcsd);
}
...
/****************************************************************************
 * Name:  ulx3s_automount_initialize
 *
 * Description:
...
int ulx3s_automount_initialize(void)
{
  void *handle;
...
void ulx3s_automount_event(int slotno, bool inserted)
{
  const struct ulx3s_automount_config_s *config;
  struct ulx3s_automount_state_s *state;
