/****************************************************************************
 * boards/risc-v/sty32c2/ulx3s/src/ulx3s_bringup.c
 *
...
int ulx3s_bringup(void)
{
  ...
  ret = ulx3s_automount_initialize();
  if (ret < 0)
    {
      syslog(LOG_ERR, "ERROR: ulx3s_automount_initialize() failed: %d\n",
              ret);
    }
#endif
  ...
  ret = ulx3s_sdio_initialize();
  if (ret != OK)
    {
      syslog(LOG_ERR, "ulx3s_sdio_initialize() failed %d\n", ret);
      return ret;
    }
