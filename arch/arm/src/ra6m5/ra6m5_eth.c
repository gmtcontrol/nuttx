/****************************************************************************
 * arch/arm/src/ra6m5/ra6m5_eth.c
 *
...
#if defined(CONFIG_NET) && defined(CONFIG_RA6M5_EMAC)
...
#define RA6M5_EMAC0_DEVNAME "eth0"
...
#if !defined(CONFIG_RA6M5_EMAC0_MII) && !defined(CONFIG_RA6M5_EMAC0_RMII)
#  warning "Neither CONFIG_RA6M5_EMAC0_MII nor CONFIG_RA6M5_EMAC0_RMII defined"
#endif

#if defined(CONFIG_RA6M5_EMAC0_MII) && defined(CONFIG_RA6M5_EMAC0_RMII)
#  error "Both CONFIG_RA6M5_EMAC0_MII and CONFIG_RA6M5_EMAC0_RMII defined"
#endif

#ifdef CONFIG_RA6M5_EMAC0_AUTONEG
#  ifndef CONFIG_RA6M5_EMAC0_PHYSR
#    error "CONFIG_RA6M5_EMAC0_PHYSR must be defined in the NuttX configuration"
#  endif
#  ifdef CONFIG_RA6M5_EMAC0_PHYSR_ALTCONFIG
#    ifndef CONFIG_RA6M5_EMAC0_PHYSR_ALTMODE
#      error "CONFIG_RA6M5_EMAC0_PHYSR_ALTMODE must be defined in the NuttX configuration"
#    endif
#    ifndef CONFIG_RA6M5_EMAC0_PHYSR_10HD
#      error "CONFIG_RA6M5_EMAC0_PHYSR_10HD must be defined in the NuttX configuration"
#    endif
#    ifndef CONFIG_RA6M5_EMAC0_PHYSR_100HD
#      error "CONFIG_RA6M5_EMAC0_PHYSR_100HD must be defined in the NuttX configuration"
#    endif
#    ifndef CONFIG_RA6M5_EMAC0_PHYSR_10FD
#      error "CONFIG_RA6M5_EMAC0_PHYSR_10FD must be defined in the NuttX configuration"
#    endif
#    ifndef CONFIG_RA6M5_EMAC0_PHYSR_100FD
#      error "CONFIG_RA6M5_EMAC0_PHYSR_100FD must be defined in the NuttX configuration"
#    endif
#  else
#    ifndef CONFIG_RA6M5_EMAC0_PHYSR_SPEED
#      error "CONFIG_RA6M5_EMAC0_PHYSR_SPEED must be defined in the NuttX configuration"
#    endif
#    ifndef CONFIG_RA6M5_EMAC0_PHYSR_100MBPS
#      error "CONFIG_RA6M5_EMAC0_PHYSR_100MBPS must be defined in the NuttX configuration"
#    endif
#    ifndef CONFIG_RA6M5_EMAC0_PHYSR_MODE
#      error "CONFIG_RA6M5_EMAC0_PHYSR_MODE must be defined in the NuttX configuration"
#    endif
#    ifndef CONFIG_RA6M5_EMAC0_PHYSR_FULLDUPLEX
#      error "CONFIG_RA6M5_EMAC0_PHYSR_FULLDUPLEX must be defined in the NuttX configuration"
#    endif
#  endif
#endif
...
#undef CONFIG_RA6M5_ETH_ENHANCEDDESC
#undef CONFIG_RA6M5_ETH_HWCHECKSUM
...
#ifndef CONFIG_RA6M5_ETH_BUFSIZE
#  define CONFIG_RA6M5_ETH_BUFSIZE OPTIMAL_ETH_BUFSIZE
#endif

#if (CONFIG_RA6M5_ETH_BUFSIZE & 31) != 0
#  error "CONFIG_RA6M5_ETH_BUFSIZE must be aligned"
#endif

#if CONFIG_RA6M5_ETH_BUFSIZE != OPTIMAL_ETH_BUFSIZE
#  warning "You using an incomplete/untested configuration"
#endif

#ifndef CONFIG_RA6M5_ETH_NRXDESC
#  define CONFIG_RA6M5_ETH_NRXDESC 16
#endif
#ifndef CONFIG_RA6M5_ETH_NTXDESC
#  define CONFIG_RA6M5_ETH_NTXDESC 12
#endif

/* We need at least one more free buffer than transmit buffers */

#define RA6M5_ETH_NFREEBUFFERS (CONFIG_RA6M5_ETH_NTXDESC+1)
...
#  undef CONFIG_RA6M5_EMAC_REGDEBUG
#endif
...
/* The ra6m5_ethmac_s encapsulates all state information for a single
 * hardware interface
 */

struct ra6m5_ethmac_s
{
  /* Descriptor allocations */

  struct eth_rxdesc_s rxtable[CONFIG_RA6M5_ETH_NRXDESC];
  struct eth_txdesc_s txtable[CONFIG_RA6M5_ETH_NTXDESC];

  /* Buffer allocations */

  uint8_t rxbuffer[CONFIG_RA6M5_ETH_NRXDESC*CONFIG_RA6M5_ETH_BUFSIZE];
  uint8_t alloc[RA6M5_ETH_NFREEBUFFERS*CONFIG_RA6M5_ETH_BUFSIZE];
...
NX_ALIGN32 static struct ra6m5_ethmac_s g_rx65nethmac[RA6M5_NETHERNET];
...
#if defined(CONFIG_RA6M5_EMAC_REGDEBUG) && defined(CONFIG_DEBUG_FEATURES)
static uint32_t ra6m5_getreg(uint32_t addr);
static void ra6m5_putreg(uint32_t val, uint32_t addr);
static void ra6m5_checksetup(void);
#else
#  define ra6m5_getreg(addr)     getreg32(addr)
#  define ra6m5_putreg(val,addr) putreg32(val,addr)
#  define ra6m5_checksetup()
#endif
...
static void ra6m5_initbuffer(struct ra6m5_ethmac_s *priv);
static inline uint8_t *ra6m5_allocbuffer(struct ra6m5_ethmac_s *priv);
static inline void ra6m5_freebuffer(struct ra6m5_ethmac_s *priv,
                                    uint8_t *buffer);
static inline bool ra6m5_isfreebuffer(struct ra6m5_ethmac_s *priv);

/* Common TX logic */

static int  ra6m5_transmit(struct ra6m5_ethmac_s *priv);
static int  ra6m5_txpoll(struct net_driver_s *dev);
static void ra6m5_dopoll(struct ra6m5_ethmac_s *priv);

/* Interrupt handling */

static void ra6m5_enableint(struct ra6m5_ethmac_s *priv,
                            uint32_t ierbit);
static void ra6m5_disableint(struct ra6m5_ethmac_s *priv,
                             uint32_t ierbit);

static void ra6m5_freesegment(struct ra6m5_ethmac_s *priv,
                              struct eth_rxdesc_s *rxfirst, int segments);
static int  ra6m5_recvframe(struct ra6m5_ethmac_s *priv);
static void ra6m5_receive(struct ra6m5_ethmac_s *priv);
static void ra6m5_freeframe(struct ra6m5_ethmac_s *priv);
static void ra6m5_txdone(struct ra6m5_ethmac_s *priv);
...
static int  ra6m5_ifup(struct net_driver_s *dev);
static int  ra6m5_ifdown(struct net_driver_s *dev);

static void ra6m5_txavail_work(void *arg);
static int  ra6m5_txavail(struct net_driver_s *dev);

#if defined(CONFIG_NET_MCASTGROUP) || defined(CONFIG_NET_ICMPv6)
static int  ra6m5_addmac(struct net_driver_s *dev, const uint8_t *mac);
#endif
#ifdef CONFIG_NET_MCASTGROUP
static int  ra6m5_rmmac(struct net_driver_s *dev, const uint8_t *mac);
#endif
#ifdef CONFIG_NETDEV_IOCTL
static int  ra6m5_ioctl(struct net_driver_s *dev, int cmd,
                        unsigned long arg);
#endif

/* Descriptor Initialization */

static void ra6m5_txdescinit(struct ra6m5_ethmac_s *priv);
static void ra6m5_rxdescinit(struct ra6m5_ethmac_s *priv);
...
static int  ra6m5_ethreset(struct ra6m5_ethmac_s *priv);
static int  ra6m5_macconfig(struct ra6m5_ethmac_s *priv);
static void ra6m5_macaddress(struct ra6m5_ethmac_s *priv);
static int  ra6m5_ethconfig(struct ra6m5_ethmac_s *priv);

static void ra6m5_phy_preamble(void);
static void ra6m5_phy_trans_zto0(void);
static void ra6m5_phy_trans_1to0(void);
static void ra6m5_phy_reg_set(uint8_t phydevaddr, uint16_t reg_addr,
                              int32_t option);
static void ra6m5_phy_reg_write(uint16_t data);
static void ra6m5_phy_mii_write1(void);
static void ra6m5_phy_mii_write0(void);

void ra6m5_ether_enable_icu(void);
void ra6m5_power_on_control(void);
void ra6m5_ether_set_phy_mode(uint8_t mode);
void ra6m5_ether_interrupt_init(void);

static int ra6m5_phywrite(uint8_t phydevaddr, uint16_t reg_addr,
                          uint16_t data);
static uint16_t ra6m5_phyread(uint8_t phydevaddr, uint16_t reg_addr,
                              uint16_t *value);
...
#ifdef CONFIG_RA6M5_EMAC_REGDEBUG
static uint32_t ra6m5_getreg(uint32_t addr)
{
...
static void ra6m5_initbuffer(struct ra6m5_ethmac_s *priv)
{
  ...
  for (i = 0, buffer = priv->alloc;
       i < RA6M5_ETH_NFREEBUFFERS;
       i++, buffer += CONFIG_RA6M5_ETH_BUFSIZE)
    {
...
static int ra6m5_transmit(struct ra6m5_ethmac_s *priv)
{
...
#if OPTIMAL_ETH_BUFSIZE > CONFIG_RA6M5_ETH_BUFSIZE
  uint8_t *buffer;
  int bufcount;
  int lastsize;
  int i;
...
#if OPTIMAL_ETH_BUFSIZE > CONFIG_RA6M5_ETH_BUFSIZE
  if (priv->dev.d_len > CONFIG_RA6M5_ETH_BUFSIZE)
    {
      /* Yes... how many buffers will be need to send the packet? */

      bufcount = (priv->dev.d_len +
                 (CONFIG_RA6M5_ETH_BUFSIZE - 1)) / CONFIG_RA6M5_ETH_BUFSIZE;
      lastsize = priv->dev.d_len - (bufcount - 1) * CONFIG_RA6M5_ETH_BUFSIZE;
      ...
      for (i = 0; i < bufcount; i++)
        {
          ...
          if (i == (bufcount - 1))
          ...
          else
            {
              ...
              txdesc->tdes1  = (CONFIG_RA6M5_ETH_BUFSIZE << 16);
              buffer        += CONFIG_RA6M5_ETH_BUFSIZE;
            }

          /* Give the descriptor to DMA */

          txdesc->tdes0 |= TACT;
          regval  = ra6m5_getreg(RA6M5_ETHD_EDTRR);
          regval |= (ETHD_EDRRR_TR);
          ra6m5_putreg(regval, RA6M5_ETHD_EDTRR);
  ...
  if (priv->inflight >= CONFIG_RA6M5_ETH_NTXDESC)
    {
      ra6m5_disableint(priv, ETHD_EESIPR_FRIP);
    }
...
static int ra6m5_txpoll(struct net_driver_s *dev)
{
   ...
   * because ra6m5_freeframe() has not yet run. If ra6m5_freeframe()
   * has run, the buffer1 pointer (tdes2) will be nullified (and
   * inflight should be CONFIG_RA6M5_ETH_NTXDESC).
   */
  ...
  dev->d_buf = ra6m5_allocbuffer(priv);
...
/****************************************************************************
 * Function: ra6m5_dopoll
 *
 * Description:
 *   The function is called in order to perform an out-of-sequence TX poll.
 *   This is done:
 *
 *   1. After completion of a transmission (ra6m5_txdone),
 *   2. When new TX data is available (ra6m5_txavail_process), and
 *   3. After a TX timeout to restart the sending process
 *      (ra6m5_txtimeout_process).
 *
 * Input Parameters:
 *   priv  - Reference to the driver state structure
 *
          ...
          if (dev->d_buf)
            {
              ...
              ra6m5_freebuffer(priv, dev->d_buf);
              dev->d_buf = NULL;
            }
        }
    }
...
static void ra6m5_enableint(struct ra6m5_ethmac_s *priv, uint32_t ierbit)
{
  uint32_t regval;

  /* ETHERC/EDMAC enabling Status Interrupt */

  regval  = ra6m5_getreg(RA6M5_ETHD_EESIPR);
  regval |= (ierbit); /* enabling ETHD_EESIPR_FRIP or ETHD_EESIPR_TCIP */

  ra6m5_putreg(regval, RA6M5_ETHD_EESIPR);
}

/****************************************************************************
 * Function: ra6m5_disableint
 *
 * Description:
                              ...
                              struct eth_rxdesc_s *rxfirst, int segments)
{
  ...
  if (!(ra6m5_getreg(RA6M5_ETHD_EDRRR)))
    {
      regval  = ra6m5_getreg(RA6M5_ETHD_EDRRR);
      regval |= (ETHD_EDRRR_RR);
      ra6m5_putreg(regval, RA6M5_ETHD_EDRRR);
    }
...
static int ra6m5_recvframe(struct ra6m5_ethmac_s *priv)
{
  ...
  for (i = 0;
       (rxdesc->rdes0 & RACT) == 0 &&
        i < CONFIG_RA6M5_ETH_NRXDESC &&
        priv->inflight < CONFIG_RA6M5_ETH_NTXDESC;
       i++)
    {
  ...
  (priv->dev.d_statistics.rx_packets)++;
              ...
              ra6m5_freesegment(priv, rxcurr, priv->segments);
...
static void ra6m5_receive(struct ra6m5_ethmac_s *priv)
{
  ...
  /* Loop while while ra6m5_recvframe() successfully retrieves valid
   * Ethernet frames.
   */

  while (ra6m5_recvframe(priv) == OK)
    {
          ...
          if ((txdesc->tdes0 & TFP1) != 0)
            {
              ...
              ra6m5_freebuffer(priv, (uint8_t *)txdesc->tdes2);
            }
          ...
          if ((txdesc->tdes0 & TFP0) != 0)
            {
               ...
               * ra6m5_enableint function
               */

              ra6m5_enableint(priv, ETHD_EESIPR_FRIP);
  ...
  if (priv->inflight <= 0)
    {
      ...
      ra6m5_disableint(priv, ETHD_EESIPR_TCIP);
    }
...
static void ra6m5_interrupt_work(void *arg)
{
  struct ra6m5_ethmac_s *priv = (struct ra6m5_ethmac_s *)arg;
  uint32_t edmasr;
  uint32_t regval;
  ...
  edmasr = ra6m5_getreg(RA6M5_ETHD_EESR);
  ...
  edmasr &= ra6m5_getreg(RA6M5_ETHD_EESIPR);
  ...
  if ((edmasr & ETHD_EESR_FR) != 0)
    {
      ...
      ra6m5_putreg(ETHD_EESR_FR, RA6M5_ETHD_EESR);
   ...
   * If so, call ra6m5_txdone(). This may disable further
  ...
  if ((edmasr & ETHD_EESR_TC) != 0)
    {
      ...
      ra6m5_putreg(ETHD_EESR_TC, RA6M5_ETHD_EESR);
  ...
  up_enable_irq(RA6M5_IRQ_ETH);
}

/****************************************************************************
 * Function: ra6m5_interrupt
 *
 * Description:
  ...
  if (edmasr != 0)
    {
      ...
      work_queue(ETHWORK, &priv->irqwork, ra6m5_interrupt_work, priv, 0);
    }
...
void ra6m5_txtimeout_expiry(wdparm_t arg)
{
   ...
   *
   * Interrupts will be re-enabled when ra6m5_ifup() is called.
   */
  ...
  work_queue(ETHWORK, &priv->irqwork, ra6m5_txtimeout_work, priv, 0);
}
...
static int ra6m5_ifdown(struct net_driver_s *dev)
{
   ...
   * the ra6m5_ifup() always successfully brings the interface back up.
   */

  ret = ra6m5_ethreset(priv);
  if (ret < 0)
    {
      nerr("ERROR: ra6m5_ethreset failed (timeout), "
  ...
  if (work_available(&priv->pollwork))
    {
      ...
      work_queue(ETHWORK, &priv->pollwork, ra6m5_txavail_work, priv, 0);
    }
...
static int ra6m5_addmac(struct net_driver_s *dev, const uint8_t *mac)
{
  ...
  /* RA6M5 do not support add on mac multi cast feature because related
   * hash table mac multi cast register is not supported in RA6M5.
   */
...
static void ra6m5_txdescinit(struct ra6m5_ethmac_s *priv)
{
  ...
  for (i = 0; i < CONFIG_RA6M5_ETH_NTXDESC; i++)
    {
      ...
      if (i < (CONFIG_RA6M5_ETH_NTXDESC - 1))
        {
  ...
  ra6m5_putreg((uint32_t)priv->txtable, RA6M5_ETHD_TDLAR);
}

/****************************************************************************
 * Function: ra6m5_rxdescinit
 *
 * Description:
...
static void ra6m5_rxdescinit(struct ra6m5_ethmac_s *priv)
{
  ...
  for (i = 0; i < CONFIG_RA6M5_ETH_NRXDESC; i++)
    {
      ...
      rxdesc->rdes1 = (uint32_t)(CONFIG_RA6M5_ETH_BUFSIZE << 16);

      /* Set Buffer1 address pointer */

      rxdesc->rdes2 = (uint32_t)&priv->rxbuffer[i*CONFIG_RA6M5_ETH_BUFSIZE];

      /* Initialize the next descriptor with the Next Descriptor Polling
       * Enable
       */

      if (i < (CONFIG_RA6M5_ETH_NRXDESC - 1))
        {
  ...
  ra6m5_putreg((uint32_t)priv->rxtable, RA6M5_ETHD_RDLAR);
}

/****************************************************************************
 * Function: ra6m5_ioctl
 *
 * Description:
      ...
      case SIOCGMIIREG: /* Get register from MII PHY */
        {
          struct mii_ioctl_data_s *req =
                 (struct mii_ioctl_data_s *)((uintptr_t)arg);
          ret = ra6m5_phyread(req->phy_id, req->reg_num, &req->val_out);
        }
        break;
      ...
      case SIOCSMIIREG: /* Set register in MII PHY */
        {
          struct mii_ioctl_data_s *req =
                 (struct mii_ioctl_data_s *)((uintptr_t)arg);
          ret = ra6m5_phywrite(req->phy_id, req->reg_num, req->val_in);
        }
        break;
...
void phy_start_autonegotiate(struct ra6m5_ethmac_s *priv, uint8_t pause)
{
  ...
  ra6m5_phywrite(priv->phyaddr,
                           PHY_REG_AN_ADVERTISEMENT, regval);
  ra6m5_phywrite(priv->phyaddr, PHY_REG_CONTROL,
                      (PHY_CONTROL_AN_ENABLE | PHY_CONTROL_AN_RESTART));
}

/****************************************************************************
 * Function Name: ra6m5_power_on_control
...
/****************************************************************************
 * Function Name: ra6m5_ether_set_phy_mode
  ...
  if (PHY_MII_SET_MODE == mode)
    {
      ...
      putreg8(ETH_PFENET_MII_MODE, RA6M5_MPC_PFENET);
    }
  else if (PHY_RMII_SET_MODE == mode)
    {
      ...
      putreg8(ETH_PFENET_RMII_MODE, RA6M5_MPC_PFENET);
    }
  else
    {
  ...
  while (databitcnt > 0) /* reading 1 bit per loop */
    {
      for (j = ETHER_CFG_PHY_MII_WAIT; j > 0; j--)
        {
          ...
          ra6m5_putreg(ETH_PIR_RESET_ALL, RA6M5_ETH_PIR);
        }
      ...
      for (j = ETHER_CFG_PHY_MII_WAIT; j > 0; j--)
        {
          ...
          ra6m5_putreg(ETH_PIR_SET_MDC, RA6M5_ETH_PIR);
        }
      ...
      reg_data |= (uint16_t) (((ra6m5_getreg(RA6M5_ETH_PIR))
                                                  & ETH_PIR_MDI) >> 3);
      ...
      for (j = ETHER_CFG_PHY_MII_WAIT; j > 0; j--)
        {
          ra6m5_putreg(ETH_PIR_RESET_ALL, RA6M5_ETH_PIR); /* Reset All Flags of PIR */
        }
                              ...
                              uint16_t *value)
{
  ...
  ra6m5_phy_preamble();
  ra6m5_phy_reg_set(phydevaddr, reg_addr, PHY_MII_READ);
  ra6m5_phy_trans_zto0();
  phy_reg_read(&data);
  ra6m5_phy_trans_zto0();
  *value = data;
  return OK;
...
/****************************************************************************
 * Function Name: ra6m5_ether_enable_icu
 * Description  :
 * Arguments    :
 * Return Value : none
 ****************************************************************************/
...
/****************************************************************************
 * Function Name: ra6m5_phy_trans_zto0
...
static void ra6m5_phy_trans_zto0(void)
{
  ...
  for (j = ETHER_CFG_PHY_MII_WAIT; j > 0; j--)
    {
      ra6m5_putreg(ETH_PIR_SET_MDC, RA6M5_ETH_PIR); /* Setting MDC of PIR */
    }
...
/****************************************************************************
 * Function Name: ra6m5_phy_trans_1to0
...
static void ra6m5_phy_trans_1to0(void)
{
  ...
  ra6m5_phy_mii_write1();
  ra6m5_phy_mii_write0();
}

/****************************************************************************
 * Function Name: ra6m5_phy_reg_set
...
/****************************************************************************
 * Function Name: ra6m5_phy_reg_write
...
/****************************************************************************
 * Function Name: ra6m5_phy_mii_write1
  ...
  for (j = ETHER_CFG_PHY_MII_WAIT; j > 0; j--)
    {
      /* Setting MDO and MMD and by default MDI */
       ...
       ra6m5_putreg(ETH_PIR_SET_MDO_MMD, RA6M5_ETH_PIR);
    }
  ...
  for (j = ETHER_CFG_PHY_MII_WAIT; j > 0; j--)
    {
      ...
      ra6m5_putreg(ETH_PIR_SET_MDO_MMD_MDC, RA6M5_ETH_PIR);
    }
...
/****************************************************************************
 * Function Name: ra6m5_phy_mii_write0
  ...
  for (j = ETHER_CFG_PHY_MII_WAIT; j > 0; j--)
    {
      ...
      ra6m5_putreg(ETH_PIR_SET_MMD, RA6M5_ETH_PIR);
    }
  ...
  for (j = ETHER_CFG_PHY_MII_WAIT; j > 0; j--)
    {
      ...
      ra6m5_putreg(ETH_PIR_SET_MMD_MDC, RA6M5_ETH_PIR);
    }
...
/****************************************************************************
 * Function Name: ra6m5_phy_preamble
                           ...
                           uint16_t data)
{
  ...
  ra6m5_phy_preamble();
  ra6m5_phy_reg_set(phydevaddr, reg_addr, PHY_MII_WRITE);
  ra6m5_phy_trans_1to0();
  ra6m5_phy_reg_write(data);
  ra6m5_phy_trans_zto0();
  return OK;
}

/****************************************************************************
 * Function Name: ra6m5_ether_interrupt_init
...
void ra6m5_ether_interrupt_init(void)
{
  ...
  ra6m5_ether_enable_icu();

  /* Clear all ETHERC status BFR, PSRTO, LCHNG, MPD, ICD */

  ra6m5_putreg(ETH_ECSR_CLR, RA6M5_ETH_ECSR);

  /* Clear all EDMAC status bits */

  ra6m5_putreg(ETHD_EESR_EDMAC, RA6M5_ETHD_EESR);

  /* Notification of ET0_LINKSTA signal change interrupt is disabled. */

  regval  = ra6m5_getreg(RA6M5_ETH_ECSIPR);
  regval &= (~(ETH_ECSIPR_LCHNGIP));
  ra6m5_putreg(regval, RA6M5_ETH_ECSIPR);
...
static int ra6m5_phyinit(struct ra6m5_ethmac_s *priv)
{
  ...
  /*  Software Reset the PHY */
    ...
    ret = ra6m5_phywrite(priv->phyaddr,
  ...
  do
    {
      ret = ra6m5_phyread(priv->phyaddr,
                          PHY_REG_CONTROL, &reg);
      count++;
    }
  ...
  phy_start_autonegotiate (priv, PAUSE_FRAME_DISABLE);

  /* Interrupt initialization */

  ra6m5_ether_interrupt_init();

  /* To check the Link status in PHY registers read Auto-negotiation
   * Advertisement register
   */

  ret = ra6m5_phyread(priv->phyaddr, PHY_REG_STATUS, &reg);
  ...
  switch (reg & CONFIG_RA6M5_EMAC0_PHYSR_ALTMODE)
    {
      default:
      case CONFIG_RA6M5_EMAC0_PHYSR_10HD:
        priv->fduplex = 0;
        priv->mbps100 = 0;
        break;

      case CONFIG_RA6M5_EMAC0_PHYSR_100HD:
        priv->fduplex = 0;
        priv->mbps100 = 1;
        break;

      case CONFIG_RA6M5_EMAC0_PHYSR_10FD:
        priv->fduplex = 1;
        priv->mbps100 = 0;
        break;

      case CONFIG_RA6M5_EMAC0_PHYSR_100FD:
        priv->fduplex = 1;
        priv->mbps100 = 1;
        break;
    }
...
static int ra6m5_ethreset(struct ra6m5_ethmac_s *priv)
{
  ...
  /* ra6m5_putreg(ETH_ECMR_CLR, RA6M5_ETH_ECMR); */

  regval  = ra6m5_getreg(RA6M5_ETH_ECMR);
  regval &= (~(ETH_ECMR_TE | ETH_ECMR_RE));
  ra6m5_putreg(regval, RA6M5_ETH_ECMR);
  ...
  regval  = ra6m5_getreg(RA6M5_ETHD_EDMR);
  regval |= ETHD_EDMR_SWR;
  ra6m5_putreg(regval, RA6M5_ETHD_EDMR);
  ...
  retries = 10;
  while (((ra6m5_getreg(RA6M5_ETHD_EDMR) & ETHD_EDMR_SWR) != 0) &&
         retries > 0)
    {
      retries--;
      up_mdelay(10);
...
static int ra6m5_macconfig(struct ra6m5_ethmac_s *priv)
{
  ...
  ra6m5_macaddress(priv);

  return OK;
}

/****************************************************************************
 * Function: ra6m5_macaddress
 *
 * Description:
...
static void ra6m5_macaddress(struct ra6m5_ethmac_s *priv)
{
  ...
  ra6m5_putreg(regval, RA6M5_ETH_MAHR);
  ...
  ra6m5_putreg(regval, RA6M5_ETH_MALR);
}

/****************************************************************************
 * Function: ra6m5_ethconfig
 *
 * Description:
...
static int ra6m5_ethconfig(struct ra6m5_ethmac_s *priv)
{
  ...
  ra6m5_power_on_control();

  /* Initialize the PHY */

  ninfo("Initialize the PHY\n");
  ret = ra6m5_phyinit(priv);
  if (ret < 0)
    {
      return ret;
    }
  ...
  ra6m5_initbuffer(priv);

  /* Initialize TX Descriptors list: Chain Mode */

  ra6m5_txdescinit(priv);

  /* Initialize RX Descriptors list: Chain Mode  */

  ra6m5_rxdescinit(priv);
  ...
  ra6m5_putreg(ETH_RFLR_RFL, RA6M5_ETH_RFLR);

  /* Intergap is 96-bit time */

  ra6m5_putreg(ETH_IPGR_IPG_INITIAL, RA6M5_ETH_IPGR);

  /* Continuous reception number of Broadcast frame */

  regval  = ra6m5_getreg(RA6M5_ETH_BCFRR);
  regval |= (ETH_BCFRR_BCF);
  ra6m5_putreg(regval, RA6M5_ETH_BCFRR);

  regval  = ra6m5_getreg(RA6M5_ETHD_TRSCER);
   ...
   */
      ...
      ra6m5_putreg(regval, RA6M5_ETHD_TRSCER);
    }
  else
    {
  ...
  ra6m5_putreg(ETHD_TFTR_TFT, RA6M5_ETHD_TFTR); /* Store and forward mode */

  /* transmit fifo is 1968 bytes & receive fifo is 2048 bytes */

  regval  = ra6m5_getreg(RA6M5_ETHD_FDR);
  regval |= ((ETHD_FDR_RFD) | (ETHD_FDR_TFD));
  ra6m5_putreg(regval, RA6M5_ETHD_FDR);
  ...
  regval  = ra6m5_getreg(RA6M5_ETHD_RMCR);
  regval |= (ETHD_RMCR_RNR);
  ra6m5_putreg(regval, RA6M5_ETHD_RMCR);

  /* Transmit Interrupt Setting */

  regval  = ra6m5_getreg(RA6M5_ETHD_TRIMD);
  regval |= ((ETHD_TRIMD_TIS) | (ETHD_TRIMD_TIM));
  ra6m5_putreg(regval, RA6M5_ETHD_TRIMD);
...
int ra6m5_ethinitialize(int intf)
{
  ...
  DEBUGASSERT(intf < RA6M5_NETHERNET);

  priv = &g_rx65nethmac[intf];

  /* Initialize the driver structure */

  memset(priv, 0, sizeof(struct ra6m5_ethmac_s));

  /* I/F up (new IP address) callback */

  priv->dev.d_ifup    = ra6m5_ifup;

  /* I/F down callback */

  priv->dev.d_ifdown  = ra6m5_ifdown;
  priv->dev.d_txavail = ra6m5_txavail;  /* New TX data callback */
#ifdef CONFIG_NET_MCASTGROUP
  priv->dev.d_addmac  = ra6m5_addmac;   /* Add multicast MAC address */
  priv->dev.d_rmmac   = ra6m5_rmmac;    /* Remove multicast MAC address */
#endif
#ifdef CONFIG_NETDEV_IOCTL
  priv->dev.d_ioctl   = ra6m5_ioctl;    /* Support PHY ioctl() calls */
#endif
  ...
  if (irq_attach(RA6M5_IRQ_ETH, ra6m5_interrupt, NULL))
    {
 ...
 *   low-level initialization logic in up_initialize.c.  If RA6M5_NETHERNET
 *   greater than one, then board specific logic will have to supply a
 *   version of arm_netinitialize() that calls ra6m5_ethinitialize() with
...
#if defined(CONFIG_RA6M5_EMAC0)

  /* Initialize the EMAC0 driver */

  ret = ra6m5_ethinitialize(EMAC0_INTF);
  if (ret < 0)
    {
...
#endif
#endif /* RA6M5_NETHERNET > 0 */
#endif /* CONFIG_NET && CONFIG_RA6M5_EMAC */
