/****************************************************************************
 * arch/arm/src/ra6m5/ra6m5_dtc.h
 *
...
#ifndef __ARCH_RENESAS_SRC_RA6M5_RA6M5_DTC_H
#define __ARCH_RENESAS_SRC_RA6M5_RA6M5_DTC_H
...
{
  DTC_TRANSFER_MODE_NORMAL,         /* Normal mode */
  DTC_TRANSFER_MODE_REPEAT,         /* Repeat mode */
  DTC_TRANSFER_MODE_BLOCK           /* Block mode */
} dtc_transfer_mode_t;
...
{
  DTC_DATA_SIZE_BYTE,               /* 8-bit (byte) data */
  DTC_DATA_SIZE_WORD,               /* 16-bit (word) data */
  DTC_DATA_SIZE_LWORD               /* 32-bit (long word) data */
} dtc_data_size_t;
...
{
  DTC_SRC_ADDR_FIXED = 0,           /* Source address is fixed. */
  DTC_SRC_ADDR_INCR  = 2,           /* Source address is incremented after each transfer. */
  DTC_SRC_ADDR_DECR  = 3            /* Source address is decremented after each transfer. */
} dtc_src_addr_mode_t;
...
{
  DTC_CHAIN_TRANSFER_DISABLE,       /* Disable Chain transfer. */
  DTC_CHAIN_TRANSFER_ENABLE         /* Enable Chain transfer. */
...
{
  DTC_CHAIN_TRANSFER_CONTINUOUSLY,  /* Chain transfer is performed continuously. */
  DTC_CHAIN_TRANSFER_NORMAL         /* Chain transfer is performed only when the counter is changed to 0 or CRAH. */
...
{
  DTC_INT_AFTER_ALL_COMPLETE,       /* Interrupt is generated when specified data transfer is completed. */
  DTC_INT_PER_SINGLE_TRANSFER       /* Interrupt is generated when each transfer time is completed. */
} dtc_interrupt_t;
...
{
  DTC_REPEAT_BLOCK_DST,             /* Destination is repeat or block area. */
  DTC_REPEAT_BLOCK_SRC              /* Source is repeat or block area. */
...
{
  DTC_DES_ADDR_FIXED = 1,           /* Destination address is fixed. */
  DTC_DES_ADDR_INCR  = 2,           /* Destination address is incremented after each transfer. */
  DTC_DES_ADDR_DECR  = 3            /* Destination address is decremented after each transfer. */
...
{
  DTC_WRITEBACK_ENABLE,
  DTC_WRITEBACK_DISABLE
} dtc_write_back_t;
...
typedef uint32_t dtc_activation_source_t;
 ...
 *
 * Input Parameters:
 *   chan - Identifies the channel resource. For the RA6M5, this
...
#endif /* __ASSEMBLY__ */
#endif /* __ARCH_RENESAS_SRC_RA6M5_RA6M5_DTC_H */
